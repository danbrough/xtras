...
import org.danbrough.xtras.Constants.Properties.PROJECT_DESCRIPTION
import org.danbrough.xtras.Constants.Properties.PROJECT_NAME
import org.danbrough.xtras.Constants.Properties.PUBLISH_DOCS
...
internal fun Project.withPublishing(block: PublishingExtension.() -> Unit) {
  ...
  runCatching {
    ...
    xError("FAILED withPublishing!! ", it)
  }
}
...
/*private fun Project.xtrasPublishToSonatype() {
  ...
  xInfo("$name::xtrasPublishToSonatype")

  registerSonatypeTasks()

  val baseURL =
    xtrasProperty<String>(SONATYPE_BASE_URL) { "https://s01.oss.sonatype.org" }
...
fun Project.xtrasPublishing() {
  ...
  withPublishing {
    publications.all {
      xtrasPom(
        projectName = xtrasPropertyValue(PROJECT_NAME) { project.name },
        projectDescription = xtrasPropertyValue(PROJECT_DESCRIPTION) {
          project.description.also {
            xWarn("$PROJECT_DESCRIPTION should be set instead of ${this@xtrasPublishing.name}.description")
          } ?: ""
        })
    }
  }

  val signPublications = xtrasPropertyValue(PUBLISH_SIGN) { true }

  if (signPublications) {
    xTrace("configuring signing..")
  ...
  if (xtrasPropertyValue<Boolean>(PUBLISH_DOCS) { true }) {
    xTrace("configuring docs..")
